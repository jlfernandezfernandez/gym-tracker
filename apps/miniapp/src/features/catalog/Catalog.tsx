...
export function Catalog() {
  ...
  const handleSelectBodyPart = (part: string) => {
    ...
    if (!part) {
      ...
      const matchingMuscle = Object.entries(MUSCLE_TO_BODYPART).find(
        ([, bp]) => bp === part,
      )?.[0];
      if (
        matchingMuscle &&
        selectedMuscle &&
        MUSCLE_TO_BODYPART[selectedMuscle] !== part
      ) {
    ...
    <>
      ...
      {showBodyMap && (
        <div class="my-3 overflow-hidden rounded-card bg-surface p-3 shadow-card transition-all">
          <div class="mb-2 flex items-center justify-between gap-2">
            <span class="min-w-0 flex-1 text-xs font-bold text-hint uppercase tracking-wider">
              Toca un músculo para filtrar
            </span>
            <button
              type="button"
              class={`grid size-9 shrink-0 place-items-center rounded-lg text-accent hover:bg-hover ${selectedMuscle ? "" : "invisible"}`}
      ...
      <div class="-mx-4 mt-3 flex gap-1.5 overflow-x-auto px-4 pb-1 [scrollbar-width:none]">
        {bodyParts.map((part) => (
          <button
            key={part}
            class={`shrink-0 cursor-pointer rounded-pill border-0 px-3 py-2 text-[.78rem] font-[650] transition active:scale-95 focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-accent ${
              bodyPart === part
                ? "bg-ink text-canvas"
      ...
      {listQuery.isLoading ? (
        ...
        <Empty icon="🔍">
          Nada con ese filtro. Prueba otro nombre o músculo.
        </Empty>
      ) : (
        <div class="mt-3 overflow-hidden rounded-card bg-surface [content-visibility:auto] [contain-intrinsic-size:auto_600px]">
          {exercises.map((exercise) => (
            <button
              key={exercise.id}
              class="grid min-h-[68px] w-full cursor-pointer grid-cols-[52px_1fr_auto] items-center gap-3 border-0 border-b border-edge bg-transparent px-[15px] py-2.5 text-left transition-colors duration-150 hover:bg-hover active:bg-hover first:rounded-t-card last:rounded-b-card last:border-b-0 focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-accent"
              onClick={() =>
                app.push({ name: "catalogExercise", exerciseId: exercise.id })
              }
            >
              <span class="grid size-[52px] place-items-center overflow-hidden rounded-xl bg-white shadow-[inset_0_0_0_1px_rgba(0,0,0,.05)]">
                {exercise.image_url ? (
                  <img
                    src={mediaUrl(exercise.image_url)}
                    alt=""
                    loading="lazy"
      ...
      {listQuery.isFetchingNextPage && (
        <p class="my-3 text-center text-xs">Cargando más...</p>
      )}
    </>
  );
}
