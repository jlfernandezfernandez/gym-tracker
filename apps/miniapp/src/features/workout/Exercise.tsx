...
export const targetValue = (target: any, exercise: any) => {
  ...
  if (executionMetric === "duration_minutes")
    return `${target.duration_minutes} min`;
  const weight = formatWeight(
    target.weight,
    exercise?.weight_mode ?? target?.weight_mode,
  );
...
export function SetRow({
  ...
  const restore = useMutation({
    mutationFn: () =>
      apiFetch(
        "POST",
        `/sessions/${sessionId}/exercises/${plannedId}/sets/restore`,
        {
          set_number: set.set_number,
          ...executionMetricPayload(
            target.execution_metric ||
              (activityType === "cardio"
                ? "duration_minutes"
                : isTimed
                  ? "duration_seconds"
  ...
  const timedExercise =
    ...
    isTimedOrIsometricExercise({
      execution_metric: target.execution_metric,
      ...target,
      ...set,
    });
      ...
      <div class="min-w-0">
        ...
        <b class="block truncate text-[.84rem]">
          {activityType === "cardio"
            ...
            : timedExercise
              ? performed
                ? `${performed} × ${set.duration_seconds}s`
...
export function Exercise({ plannedId }: { plannedId: number }) {
  ...
  const exercise = plan?.exercises?.find(
    (candidate: any) => String(candidate.planned_id) === String(plannedId),
  );
  ...
  const performedSetNumbers = new Set<number>(
    (exercise.performed_sets || []).map((set: any) => set.set_number),
  );
  ...
  const instructions =
    ...
    exercise.notes ||
    "Sigue las indicaciones del coach en Telegram.";
  const showEditor =
    canEditWorkout(app.readOnly, plan.status, exercise.status) &&
    currentSetNumber !== undefined;
  const futureSetNumbers = missingSets.filter(
    (setNumber) => setNumber !== currentSetNumber,
  );
    ...
    <>
      ...
      <div class="my-3 overflow-hidden rounded-card bg-surface shadow-card min-[720px]:grid min-[720px]:grid-cols-[1.05fr_.95fr]">
        ...
        <div class="relative grid h-[200px] w-full place-items-center overflow-hidden bg-white shadow-[inset_0_0_0_1px_rgba(0,0,0,.05)] min-[720px]:h-auto min-[720px]:min-h-[280px]">
          {mediaSrc ? (
            <img
              class="h-[180px] w-[180px] max-h-[190px] max-w-[190px] object-contain p-1"
              src={mediaSrc}
        ...
        <div class="p-[18px] min-[720px]:flex min-[720px]:flex-col min-[720px]:justify-center">
          ...
          <div class="mt-4 flex items-center justify-between text-[.68rem] font-bold tracking-[.05em] text-hint uppercase">
            <span>Progreso</span>
            <span>
              {loggedSetCount}/{exercise.sets} series
            </span>
          </div>
          <div class="mt-2">
            <SetProgress
              total={exercise.sets || 0}
              completedSetNumbers={performedSetNumbers}
              currentSetNumber={currentSetNumber}
              showCurrent={showEditor}
              ariaLabel={`${loggedSetCount} de ${exercise.sets} series completadas`}
            />
          </div>
        </div>
      </div>
      ...
      <div class="card">
        <div class="mb-3 flex min-h-9 items-center justify-between gap-3">
          ...
          {showEditor && (
            <SetCountControl
              ...
              minimumSets={Math.max(...performedSetNumbers, 0)}
            />
          )}
        </div>
        <div class="grid gap-2">
          {[...(exercise.performed_sets || [])]
            .sort(
              (first: any, second: any) => first.set_number - second.set_number,
            )
            .map((performedSet: any) => (
              <SetRow
                key={performedSet.id}
                set={performedSet}
          ...
          {futureSetNumbers.map((setNumber) => {
            return (
              <div
                key={setNumber}
                role="group"
                aria-label={`Serie ${setNumber} pendiente`}
                class="grid grid-cols-[34px_minmax(0,1fr)_auto] items-center gap-2.5 rounded-control bg-surface-2/55 px-3 py-3"
              >
                <span
                  aria-hidden="true"
                  class="grid size-[30px] place-items-center rounded-pill bg-surface-2 text-[.7rem] font-bold text-hint"
                >
                  S{setNumber}
                </span>
                <div class="min-w-0">
                  <b class="block truncate text-[.84rem] text-ink">
                    {targetValue(targetForSet(exercise, setNumber), exercise)}
                    {exercise.unilateral ? " · Unilateral" : ""}
                  </b>
                </div>
        ...
        {!app.readOnly && exercise.status === "completed" && (
          <div class="mt-3 border-t border-edge pt-3 text-center">
            ...
            <button
              class="mt-1 min-h-11 w-full cursor-pointer rounded-2xl border-0 bg-transparent px-4 py-2 text-[.9rem] font-[700] text-accent active:scale-[.975]"
              onClick={app.pop}
            >
              Volver al plan
            </button>
        ...
        <NextExercisePicker
          exercises={
            plan?.exercises?.filter(
              (e: any) =>
                e.planned_id !== exercise.planned_id &&
...
function ExerciseProgress({ exerciseId }: { exerciseId: number }) {
  ...
  const progressQuery = useQuery({
    ...
    queryFn: () =>
      apiFetch<any[]>("GET", `/exercises/${exerciseId}/progress?limit=50`),
  ...
  return (
    <div class="card">
      ...
      <div class="mt-3 flex items-center">
        <div class="flex-1">
          <span class="mb-1 block text-[.68rem] font-bold tracking-[.06em] text-hint uppercase">
            Mejor
          </span>
          <b class="text-[1.05rem]">
            {best} {unit}
          </b>
        </div>
      ...
      {points.length >= 2 && (
        <details class="mt-3 border-t border-edge pt-2 [&[open]>summary]:mb-2.5">
          ...
          <p class="text-xs">{progressSummaryText(metric)}</p>
...
function SetCountControl({
  ...
  const adjust = useMutation({
    mutationFn: (target: number) =>
      apiFetch("PUT", `/sessions/${sessionId}/exercises/${plannedId}`, {
        target_sets: target,
      }),
    onSuccess: (updated: any) => {
  ...
  return (
    <div
      ...
      aria-label={`${sets} series planificadas`}
    >
      <button
        class="grid size-9 cursor-pointer place-items-center rounded-xl border-0 bg-surface-2 text-[1.2rem] font-bold text-ink transition active:scale-90 active:bg-hover disabled:cursor-default disabled:opacity-30"
        disabled={adjust.isPending || sets <= minimumSets}
        onClick={() => step(-1)}
        aria-label="Quitar serie"
      >
        −
      </button>
      <span class="min-w-6 text-center text-[1.05rem] font-[720] tracking-[-.03em]">
        {sets}
      </span>
      <button
        class="grid size-9 cursor-pointer place-items-center rounded-xl border-0 bg-surface-2 text-[1.2rem] font-bold text-ink transition active:scale-90 active:bg-hover disabled:cursor-default disabled:opacity-30"
        disabled={adjust.isPending || sets >= 20}
        onClick={() => step(1)}
...
export function IsometricTimer({
  ...
  const progressPct =
    targetSeconds > 0 ? Math.min(100, (elapsedSec / targetSeconds) * 100) : 0;
  const displaySeconds =
    timerState === "finished" ? targetSeconds : remainingSec;

  return (
    <div
      role="region"
      aria-label="Cronómetro isométrico"
      class="my-3 rounded-2xl border border-accent/25 bg-surface p-3.5 shadow-sm"
    >
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
          <span
            class={`grid size-7 place-items-center rounded-pill text-xs ${
              timerState === "running"
                ? "bg-ok-bg text-ok animate-pulse"
...
export function LogSetForm({
  ...
  const [isWarmup, setIsWarmup] = useState(setTarget?.is_warmup || false);
  ...
  const logSet = useMutation({
    mutationFn: async ({
      ...
      const payload = {
        ...
        ...executionMetricPayload(
          ...
          {
            duration_minutes:
              metricMinutes ?? parseInt(durationMinutes || "0", 10),
            duration_seconds: metricReps,
      ...
      if (app.workoutSync?.journal) {
        const updated = await app.workoutSync.journal.enqueue(
          sessionId,
          exercise.planned_id,
          payload,
        );
      ...
      return apiFetch(
        "POST",
        `/sessions/${sessionId}/exercises/${exercise.planned_id}/sets`,
        payload,
      );
    },
    onSuccess: (updatedSession) => {
      ...
      if (isLastSet) {
        showToast(
          app.workoutSync?.journal
            ? "Series registradas; sincronizando"
            : "Ejercicio completado",
          "ok",
        );
        const pending = (updatedSession.planned_exercises || []).filter(
          (candidate: any) =>
            candidate.id !== exercise.planned_id &&
  ...
  const completeExercise = useMutation({
    mutationFn: () =>
      apiFetch(
        "POST",
        `/sessions/${sessionId}/exercises/${exercise.planned_id}/complete`,
      ),
  ...
  const saveSet = () => {
    ...
    if (isNaN(normalizedWeight) || normalizedWeight <= 0) {
      showToast(
        "El peso debe ser mayor que 0; déjalo vacío si no hay carga",
        "err",
      );
      return;
    }
  ...
  return (
    <div
      role="group"
      aria-label={`Serie ${nextSetNumber} en curso`}
      class="rounded-control border border-accent/20 bg-accent-bg/55 p-3"
    >
      <div class="mb-3 grid grid-cols-[34px_minmax(0,1fr)] items-center gap-2.5">
        <span
          aria-hidden="true"
          class={`grid size-[30px] place-items-center rounded-pill text-[.7rem] font-bold text-white ${isWarmup ? "bg-amber-500" : "bg-accent"}`}
        >
        ...
        <div class="min-w-0">
          ...
          <b class="block truncate text-[.84rem]">
            Plan · {targetValue(setTarget, exercise)}
            {exercise.unilateral ? " · Unilateral" : ""}
          </b>
        </div>
      </div>
      {isCardio ? (
        <div>
          ...
          <input
            ...
            onInput={(event: any) => setDurationMinutes(event.target.value)}
          />
        </div>
      ) : (
        <div class="flex items-stretch gap-[9px]">
          <div class="min-w-0 flex-1">
            <label for="set-weight">
              {isBodyweight ? "Peso corporal" : "Peso (kg)"}
            </label>
            <div class="flex items-center gap-1.5">
              {isBodyweight ? (
                <div class="grid min-h-12 min-w-0 flex-1 place-items-center rounded-control bg-surface text-[1rem] font-[720] tracking-[-.03em] text-hint">
                  Corporal
                </div>
              ) : (
                <input
          ...
          <div class="min-w-0 flex-1">
            ...
            <div class="flex items-center gap-1.5">
              <input
                ...
                value={reps}
                onFocus={(event: any) => event.target.select()}
                onInput={(event: any) => setReps(event.target.value)}
              />
            </div>
          </div>
        </div>
          ...
          <div class="grid grid-cols-4 gap-1.5">
            {[
              ...
              <button
                ...
                onClick={() =>
                  setSelectedRir(selectedRir === option.rir ? null : option.rir)
                }
              >
                {option.label}
              </button>
      ...
      <BusyButton
        busy={isBusy}
        busyLabel="Guardando..."
        class="btn-primary mt-4 bg-ink text-canvas disabled:pointer-events-none disabled:opacity-35"
        onClick={saveSet}
      >
      ...
      {!isLastSet && (
        <button
          class="mt-1 min-h-11 w-full cursor-pointer rounded-2xl border-0 bg-transparent px-4 py-2 text-[.88rem] font-[700] text-accent transition active:scale-[.975] disabled:pointer-events-none disabled:opacity-35"
          disabled={isBusy}
...
function NextExercisePicker({
  exercises,
  onPick,
  onDismiss,
}: {
  exercises: any[];
  onPick: (plannedId: number) => void;
  onDismiss: () => void;
}) {
  ...
  return (
    <dialog
      ref={dialogRef}
      class="native-sheet m-auto mb-2.5 w-[min(100%-20px,430px)] rounded-[26px] border border-white/50 bg-surface/94 p-5 text-ink shadow-sheet backdrop-blur-3xl backdrop-saturate-150 [&::backdrop]:bg-black/35"
      onClose={onDismiss}
    >
        ...
        {exercises.map((exercise: any) => {
          ...
          return (
            <button
              ...
              class="grid w-full cursor-pointer grid-cols-[48px_minmax(0,1fr)_auto] items-center gap-3 rounded-2xl border-0 bg-surface-2 p-3 text-left transition active:scale-[.98] focus:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 focus-visible:ring-offset-surface"
              onClick={() => onPick(exercise.planned_id)}
            >
              ...
              <div class="min-w-0">
                ...
                <p class="text-[.72rem] text-hint">
                  {formatMuscle(exercise.target || "")} ·{" "}
                  {exercise.activity_type === "cardio"
                    ? `${exercise.sets}×${exercise.duration_minutes} min`
                    : isTimed
                      ? `${exercise.sets}×${exercise.duration_seconds || exercise.target_duration_seconds || 30}s`
                      : `${exercise.sets}×${exercise.reps}`}
                </p>
              </div>
              <span aria-hidden="true" class="text-lg text-hint">
                ›
              </span>
            </button>
          );
      ...
      <div class="mt-4 border-t border-edge pt-3">
        <button
          class="min-h-[46px] w-full cursor-pointer rounded-2xl border-0 bg-transparent font-[680] text-accent transition active:scale-[.975]"
          onClick={onDismiss}
        >
          Ver plan completo
        </button>
