...
export function RecordDetail({
  ...
  const progressQuery = useQuery({
    ...
    queryFn: () =>
      apiFetch("GET", `/exercises/${exerciseId}/progress?limit=50`),
  });
  ...
  const maximum = points.length
    ? Math.max(...points.map((point: any) => progressValue(point, metric)))
    : 0;
        ...
        <>
          <div class="my-3 rounded-card bg-surface p-[18px] shadow-card">
            <div class="grid grid-cols-3 gap-[9px]">
              ...
              <Stat
                label="Última"
                value={`${progressValue(latestPoint, metric)} ${unit}`}
              />
            </div>
          </div>
          {points.length >= 2 && (
          ...
          <div class="mt-[14px] overflow-hidden rounded-card bg-surface [content-visibility:auto] [contain-intrinsic-size:auto_500px]">
            {[...points].reverse().map((point: any) => (
              <button
                class="grid min-h-[76px] w-full cursor-pointer grid-cols-[82px_1fr_auto] items-center gap-2.5 border-0 border-b border-edge bg-transparent px-[15px] py-3 text-left text-ink first:rounded-t-card last:rounded-b-card last:border-b-0 hover:bg-surface-2 active:bg-surface-2 focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-accent"
                key={point.session_id}
                onClick={() => app.openSession(point.session_id)}
              >
                <span class="text-[.74rem] text-hint">
                ...
                <span class="min-w-0">
                  <b class="block overflow-hidden text-[.9rem] text-ellipsis whitespace-nowrap">
                    ...
                    {point.sets > 1
                      ? ` · ${point.sets} ${metric === "minutes" ? "bloques" : "series"}`
                      : metric === "minutes"
                        ? " · 1 bloque"
                        : " · 1 serie"}
                  </b>
                  <small class="mt-[3px] block text-[.72rem] text-hint">
                    {metric === "weight"
                      ? `${Math.round(point.volume)} kg volumen`
                      : metric === "minutes"
                        ? `${point.top_duration_minutes} min`
                        : `${point.sets} series`}
                  </small>
                </span>
