...
const planned = (
  ...
  notes: '',
  status,
  set_targets: [1, 2, 3].map((setNumber) => ({ set_number: setNumber, weight, reps, duration_minutes: null })),
...
export async function demoFetch(method: string, path: string): Promise<any> {
  ...
  if (pathname === '/exercises/facets') {
    return {
      ...
      activity_types: [...new Set(exercises.map((exercise) => exercise.activity_type))],
    };
  }
