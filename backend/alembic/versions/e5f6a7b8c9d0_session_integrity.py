...
def upgrade() -> None:
    ...
    op.create_unique_constraint("uq_performed_set_number", "performed_sets", ["planned_exercise_id", "set_number"])
    op.create_unique_constraint("uq_planned_exercise_order", "planned_exercises", ["session_id", "order"])
    op.create_check_constraint("ck_session_status", "workout_sessions", "status IN ('planned', 'in_progress', 'completed', 'cancelled')")
    op.create_check_constraint("ck_planned_status", "planned_exercises", "status IN ('pending', 'in_progress', 'completed', 'skipped')")
